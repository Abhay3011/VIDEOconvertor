...
async def set_timer(event, list1, list2):
    ...
    await event.client.send_message(event.chat_id, 'You can start a new process again after 2 seconds.')
...
async def check_timer(event, list1, list2):
    if f'{event.sender_id}' in list1:
        ...
        present = time.time()
        return False, f"You have to wait {2-round(present-float(last))} seconds more to start a new process!"
    else:
        return True, None
...
async def fcomp(event):
    ...
    if f'{event.sender_id}' in process1:
        ...
        present = time.time()
        return await event.answer(f"You have to wait {2-round(present-float(last))} seconds more to start a new process!", alert=True)
    button = await event.get_message()
    msg = await button.get_reply_message()
